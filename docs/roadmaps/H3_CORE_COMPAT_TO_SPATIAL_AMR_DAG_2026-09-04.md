...
| Phase A status | A0/A8/A1/A2/A3/A5 DONE on branch `night-0904-int` (pushed, not merged): per-token rows, FinalLayer + PDD fallback, 4-state capability, auto-gated shim, H3 Core Compatibility node, mask-shape contract. A6 DONE 2026-09-04 daytime: tensor parity on both cores (`tests/parity_15988_cores.py`), the three H3_CKPT vram_lab gates exact on real weights, and same-seed renders identical A/V across stock+shim and the patched core with a mode-off control that differs in every frame (write-up in ModelCatalog `docs/H3_MASK_CONV_DTYPE_2026-09-04.md`). The mask-conv dtype 'deviation' was measured to be none (b408dc3). A4 DONE 2026-09-04 afternoon: temporal-insert 0/1 mask and per-token fractional audio ramp both render under H3StreamedBlocks and are bit-equal to stock at the LATENT (H3LatentBank) and pixel level; unmasked same-seed bit-equal; q_chunk must be >= S under the sage patch (tail split, self_check finds it); mp4-level A/Bs carry an x264 noise floor (~0.3-0.6/255 mean) and cannot certify equality. PDD fallback render still owed (no core-form artifact on disk). Write-up: ModelCatalog `docs/H3_STREAMED_A4_SMOKE_2026-09-04.md`. A7 DONE 2026-09-04 afternoon at the BRANCH level: `night-0904-int` merged into `mask-conversion-0903` (ffb1030, pushed; 32 commits over origin/main), every CPU gate re-run on the merged live tree, ALPHA.md section for the two compat nodes, replies for #4/#5 and the release note DRAFTED (never posted by an agent; operator posts) in ModelCatalog `docs/A7_REPLIES_AND_RELEASE_NOTE_2026-09-04.md`. Operator rulings: PR number + author stay in the docstring, compat handoff committed as received, HOLD at the branch (no merge to main, no tag) until he has looked. B0/B1/B2/C0/S1/T1/B4 are unblocked on the branch |
| S0 result (night 09-04, ModelCatalog `docs/S0_TILER_AUDIT_2026-09-04.md`) | Lab root for T1: `/mnt/work/ai/apps/ComfyUI-lab-t1` (stock 7d2640b3 worktree, production packs symlinked, MAINodes -> live tree, USDU fork fixture @ 6836cf3 with `USDU_H3_RETURN_MASK=1` = the 8360ef6 arm, `[usdu-h3-fixture]` per-tile geometry log, geometric editability from crop history; imports on CPU, 1364 nodes, no collision with the MMH3 pack). FINDINGS: `Comfyui-MMH3-UltimateUpscale` (850f4dc, production) ALREADY implements the supplement's frozen-neighbour MVP in latent space (`nodes.py:196-226` spatial_fade_mask, done strips pre-filled from the accumulated result `:904-908`, true-extent tiles, no LANCZOS, /32 at the input, sequential, audio frozen over the real sliced latent `:914/:1267`); its default `fade_width/height 32` is `torch.linspace(0,1,2)` after the pack's `// 16` (`nodes.py:864-868`) = exactly [0, 1], BINARY at the model (measured 09-04 night: 0 fractional rows in 12 tile calls, the shim on/off pair decoded-identical); >= 64 px (4 cells) is the fractional path; fade 0 = hard freeze. The fork's `ProcessedRegionTracker` is never assigned (`processing.py:203/:317`): CONTEXT_ONLY == REPROCESS for every model. The fork's batch>1 path (`usdu_patch.py:250-333`) has no H3 branch: T1 runs batch=1 for correctness. The fork's `fix_crop_region` (-1 px per interior edge) + aspect-matching expand make odd crops, LANCZOS fires on every tile, and `round_length` rounds to 8 so the H3 %32 assertion (`processing.py:570-574`) can raise: S3 is a prerequisite of the MVP geometry. Arm 2 on this core is not the author's experiment (#15375 present; audio zeros -> empty template injected clean at cond strength every step); only the `ComfyUI-pr15375` worktree reproduces it (optional arm 5). Not yet run inside a real `process_images`; the first T1 launch is the instrumentation's first test |
| B0 status (night 09-04, ModelCatalog `docs/measurements/RESULT_maskconv_*_2026-09-04.md`) | DONE at the numbers level, playback owed. (a) panrun ladder 0.10-0.90 + scope factorial at 0.5 (G0b): on-video == off and on-audio == on-both DECODED-IDENTICAL, the whole picture change rides the audio path; dilated audio rel-rms monotone 0.35 -> 0.82 to 0.75 then 0.53 at 0.90; video PSNR flat ~31 dB. (b) helpchain4link off/on (max exposure, all three mechanisms, fired log shows fractional rows for each): different takes from link 2 (16 dB, rel-rms 0.46) compounding to link 4 (10 dB, 0.87); mechanism attribution needs a per-mechanism factorial (3 cells, owed). (c) MMH3 tiler at fade 32: null of the INPUT (binary at the model, see the S0 row). (d) MMH3 tiler at fade 64: fractional rows on every tile (~10 %), audio frozen and identical, tiled stage 40.65 dB / 1.75 per 255 against a 0.90 per 255 encoder floor with the maxima inside both fade bands (x 800-864, y 768-832): CERTIFIED at the latent 23:28 (banked pair, `latent_diff.py`): stage-1 control bit-equal; tiled latent differs with an 8x spike at the FIRST fractional cell (m = 1/3) of every fade band (x 51, y 27/49), exactly zero in the first tile's chunk-1 frames where no fractional row exists, spread by attention within fade tiles and by the temporal anchors into chunk 2; audio bit-equal. First spatial evidence that the correction acts where and only where the mask is fractional. Seam quality: playback. Every replayed row has a ledger entry and its `mc_on`/`mc_off` prefix. Rules minted: copy the shim's fired log out of /tmp after every A/B; `maskconv_pair_metrics.py` (native-rate audio, ffmpeg psnr) is this family's ruler, not `audio_metrics.py`; an explicit off must strip an auto-installed wrapper (MAINodes 1db6d5e) |
...
**S0 two-repo audit** (2 h, needs A0 only). DONE 2026-09-04 night, see the S0 row in s.0; S1/S2 should consider the MMH3 pack as the fixture (it is the MVP already, in latent space) and T1 keeps the fork for the history question. Ruling 09-04: the behavioural fixture
